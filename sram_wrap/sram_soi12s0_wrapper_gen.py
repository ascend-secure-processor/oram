...
if len(sys.argv) >= 5:
	NPorts, NWords, NBits, NDec = (int(arg) for arg in sys.argv[1:6])
elif len(sys.argv) >= 4:
...
SRAM_name = "SRAM" \
	...
	+ "CSN"	# TODO: some configs less relevent to IO

SRAM_name += "%05d" % NWords + "X%03d" % NBits + "D%02d" % NDec 
...
# test ports
Row_redundancy = 0 if SRAM_name[6] == 'C' else 4
Col_redundancy = 1 << int(ceil(log(ceil(NBits * NDec / 144.0), 2)))
